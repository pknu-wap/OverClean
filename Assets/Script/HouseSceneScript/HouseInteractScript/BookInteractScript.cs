...
public class BookInteractScript : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (isPuzzleOpen)
        {
            if (PuzzleManager.instance.isPuzzleSuccess)
            {
                ...
                // RPC 함수 호출
                photonView.RPC("BookInteractRPC", RpcTarget.All);
            }
    ...
    void Interact(Transform playerLocation)
    {
        ...
        {
            // 씬매니저로 퍼즐씬 불러오기
            SceneManager.LoadScene("HouseBookPuzzleScene", LoadSceneMode.Additive);
            // 퍼즐 오픈 변수 true
            isPuzzleOpen = true;
    ...
    void BookInteractRPC()
    {
        ...
        // 태스크 카운트 업데이트
        taskUIManager.GetComponent<TaskUIManager>().UpdateCount(5);
    }
